...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 pb-2 px-3 bottom-nav:hidden">
      <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-[0_2px_16px_rgba(0,0,0,0.08)] border border-white/50 max-w-md mx-auto">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex flex-col items-center justify-center gap-0.5 w-16 h-full active:opacity-60 transition-colors ${isActive ? 'text-blue-500' : 'text-gray-400'}`}
              >
                {item.icon(isActive)}
