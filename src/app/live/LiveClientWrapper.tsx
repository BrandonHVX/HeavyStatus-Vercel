...
export default function LiveClientWrapper({
  ...
  return (
    <div className="bg-white min-h-screen max-w-lg mx-auto pb-24 relative font-['Inter',system-ui,-apple-system,sans-serif] pb-24">
          ...
          {upNextPosts.map((post, i) => (
            <div key={post.id}>
              <div className="flex items-start gap-3 py-3">
                <Link
                  href={post.href}
                  className="flex-shrink-0 w-[100px] h-[75px] rounded-xl overflow-hidden relative bg-gray-800 block"
                >
                  <Image
                    src={post.imageUrl}
                    alt={post.title}
                ...
                >
                  <svg
                    ...
                    height="20"
                    viewBox="0 0 24 24"
                    fill={savedPosts.has(post.id) ? "white" : "none"}
                    stroke={savedPosts.has(post.id) ? "white" : "#6b7280"}
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  >
