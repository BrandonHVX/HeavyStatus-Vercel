...
export default async function Home() {
  ...
  return (
    <div className="bg-white min-h-screen max-w-3xl mx-auto relative pb-20">
      {/* Scrollable content */}
      <div className="scrollbar-hide">
        ...
        <div className="mx-4 mb-5 rounded-[20px] overflow-hidden relative">
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(to bottom, rgba(0,0,0,0.08), rgba(255,255,255,0.6) 50%, rgba(255,255,255,0.97))",
            }}
          />
        ...
        <div className="flex gap-2 px-4 pb-5 overflow-x-auto scrollbar-hide">
          ...
          >
            <svg width="15" height="15" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2c0 0-5 4-5 10a5 5 0 0 0 10 0c0-2.5-1.5-5-2-6.5C14.5 7 14 9 12 10c0 0 1-4-1-8z" />
            </svg>
            Hot
          </Link>
        ...
        <div className="px-5 pb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/featured" className="text-[13px] text-gray-400">
              See all ›
            </Link>
          </div>
          <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-hide">
            {magazinePosts.map((post) => (
              <Link
                key={post.id}
                href={postHref(post)}
                className="flex-shrink-0 w-[114px] block group"
              >
                ...
                <p
                  className="mt-2 text-[13px] font-semibold text-gray-900 leading-tight line-clamp-2"
                  dangerouslySetInnerHTML={{ __html: post.title }}
                />
                <p className="text-[11px] text-gray-400 mt-0.5">
                  {fmtMonthYear(post.date)}
                </p>
              </Link>
            ))}
          </div>
        ...
        <div className="px-5 pb-6">
          ...
          {featuredPost && (
            <Link
              ...
              className="block relative rounded-2xl overflow-hidden mb-3"
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4">
                ...
                <h3
                  className="text-[16px] font-bold text-white leading-snug mb-2 line-clamp-2"
                  dangerouslySetInnerHTML={{ __html: featuredPost.title }}
                />
                <div className="flex items-center gap-2">
                  <div
                    className={`w-6 h-6 rounded-full ${getAvatarColor(postAuthor(featuredPost))} flex items-center justify-center flex-shrink-0`}
                  >
                    <span className="text-white text-[8px] font-bold">
                      {getInitials(postAuthor(featuredPost))}
                    </span>
                  </div>
            ...
            <div key={post.id}>
              ...
              >
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  ...
                  <h4
                    className="text-[14px] font-semibold text-gray-900 leading-snug line-clamp-2 mb-1.5"
                    dangerouslySetInnerHTML={{ __html: post.title }}
                  />
                  <div className="flex items-center gap-1.5">
                    <div
                      className={`w-5 h-5 rounded-full ${getAvatarColor(postAuthor(post))} flex items-center justify-center flex-shrink-0`}
                    >
                      <span className="text-white text-[7px] font-bold">
                        {getInitials(postAuthor(post))}
                      </span>
                    </div>
                    <span className="text-[11px] text-gray-400">
                      {postAuthor(post)} · {timeAgo(post.date)}
                    </span>
                  </div>
                </div>
              </Link>
                ...
                >
                  <div className="w-11 h-11 rounded-full bg-gray-200 overflow-hidden relative flex items-center justify-center flex-shrink-0">
                    {author.avatar?.url ? (
                      ...
                      <div
                        className={`w-full h-full ${getAvatarColor(author.name)} flex items-center justify-center`}
                      >
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-gray-400 mt-0.5">
                      {author.description
                        ? stripHtml(author.description, 25)
                        : "Author"}
                    </p>
                  </div>
                </Link>
